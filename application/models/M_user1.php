...
class M_user1 extends CI_Model
{
    ...
    public function tampil_data1($conditions)
    {
        $this->db->select('user.*, tb_tempatpkl.*');
        $this->db->from('user');
        $this->db->join('tb_tempatpkl', 'user.id = tb_tempatpkl.id_user');
        $this->db->where($conditions); // Pass the conditions directly
